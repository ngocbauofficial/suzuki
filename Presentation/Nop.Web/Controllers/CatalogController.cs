...
namespace Nop.Web.Controllers
{
    public partial class CatalogController : BasePublicController
    {
        ...
        public JsonResult ListName(string q)
        {
            ...
            var states = _stateProvinceService.GetStateProvincesByCountryId(country != null ? country.Id : 0, _workContext.WorkingLanguage.Id).Select(x=>x.Name).ToList();
            return Json(new
            {
                data = states,
                status = true
        ...
        public ActionResult Brochure(int? productThumbPictureSize)
        {
            ...
            var model = PrepareProductOverviewModels(productsList, true, true, productThumbPictureSize).ToList();
            return View(model);
        }
        public ActionResult FindVendor( string province)
        {
            ...
            if (province != null)
            {
                ...
                var model = states.Where(x => x.Name.Contains(province)).ToList();
                var vendor = _vendorService.GetAllVendors().ToList();

                var query = vendor.Where(delegate (Vendor c)
                {
                    if (ConvertToUnSign(c.Address).IndexOf(province, StringComparison.CurrentCultureIgnoreCase) >= 0)
                        return true;
                if (c.Address.IndexOf(province, StringComparison.CurrentCultureIgnoreCase) >= 0)
                        return true;
                    else
                        return false;
                }).AsQueryable();
                ...
                var getvendor = new List<VendorModel>();
                getvendor = (from a in query
                ...
                {
                    ViewBag.Message = "không có kết quả tìm kiếm nào với từ khóa";
                    var vendora = _vendorService.GetAllVendors().ToList();
                    getvendor = (from a in vendora
                                 select new VendorModel
                                 {
                                     Name = a.Name,
                                     Phone = a.Phone,
            ...
            {
                var vendora = _vendorService.GetAllVendors().ToList();
               var getvendor = (from a in vendora
                             select new VendorModel
                             {
                                 Name = a.Name,
                                 Phone = a.Phone,
                ...
                ViewBag.Check = "không có kết quả tìm kiếm nào với từ khóa";
                return View(getvendor);
            }
        }
        ...
        [ChildActionOnly]
        public ActionResult ManufacturerHomePageProductsOne(int? productThumbPictureSize, int manufacturerId)
        {
            var ManufacturerIds = new List<int>();
            var products = _productService.SearchProducts(manufacturerId: manufacturerId);
            if (!products.Any())
                return Content("");
            var model = PrepareProductOverviewModels(products, true, true, productThumbPictureSize).ToList();
            return PartialView("_ProductBox_Homepage_One", model);
        }
        [ChildActionOnly]
        public ActionResult ManufacturerHomePageProductsTrue(int? productThumbPictureSize, int manufacturerId)
        {
            var ManufacturerIds = new List<int>();
            var products = _productService.SearchProducts(manufacturerId: manufacturerId);
            if (!products.Any())
                return Content("");
            var model = PrepareProductOverviewModels(products, true, true, productThumbPictureSize).ToList();
            return PartialView("_ProductBox_Homepage_True", model);
        }
        public ActionResult ManufacturerHomePage()
        {
            ...
            return View("ManufacturerHomePage", model);
        }
        [ChildActionOnly]
        public ActionResult CategoryHomePageProducts(int? productThumbPictureSize, int categoryid)
        {
            var categoryIds = new List<int>();
            ...
            if (!products.Any())
                return Content("");
            var model = PrepareProductOverviewModels(products, true, true, productThumbPictureSize).ToList();
            return PartialView("_Product_Menu", model);
        }

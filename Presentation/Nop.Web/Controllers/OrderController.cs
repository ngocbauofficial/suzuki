...
namespace Nop.Web.Controllers
{
    public partial class OrderController : BasePublicController
    {
...

        {
            if (vendorid != null)
            {
                var id = vendorid.GetValueOrDefault();
       
                var vendor = _vendorService.GetVendorById(id);
                ViewBag.Google = vendor.GoogleMap;
                ViewBag.NameVendor = vendor.Name;
                ViewBag.PhoneVendor = vendor.Phone;
                ViewBag.AddressVendor = vendor.Address;
                ViewBag.EmailVendor = vendor.Email;
                return PartialView("_GetVendorDetail");
            }
            ViewBag.NameVendor = "";
            return PartialView("_GetVendorDetail");
        }

        public ActionResult GetVendors(int? StateId)
        {
            if (StateId != null)
            {
                var id = StateId.GetValueOrDefault();
                var vendors = _vendorService.GetAllVendors().Where(x => x.StateId == StateId).ToList();
                return PartialView("_GetVendors", vendors);
            }
            return Json(0);
        }
            ...
            
        {
            if (Productid != null)
            {
                var id = Productid.GetValueOrDefault();
            ...
            _mediaSettings.ProductDetailsPictureSize;
                ...
                ViewBag.Images = defaultPictureModel.ImageUrl;
                return PartialView("_Getimages");
            }
            ViewBag.Images = "";
            return PartialView("_Getimages");
        }
        public ActionResult Booking(int? productid,int? idvendor)
        {
            ...
            if (idvendor != null)
            {
                var Idvendor = idvendor.GetValueOrDefault();
                var vendor = _vendorService.GetVendorById(Idvendor);
                var allvendor = _vendorService.GetAllVendors().Where(x=>x.StateId==vendor.StateId);
                ViewBag.State = new SelectList(states, "ID", "Name",vendor.StateId);
                ViewBag.VendorId = new SelectList(allvendor, "ID", "Name", idvendor);
                ViewBag.Google = vendor.GoogleMap;
                ViewBag.NameVendor = vendor.Name;
                ViewBag.PhoneVendor = vendor.Phone;
                ViewBag.AddressVendor = vendor.Address;
            ...
            {
                var vendor = new List<VendorModel>();
              var a= new SelectList(vendor, "ID", "Name");
              
                ViewBag.State = new SelectList(states, "ID", "Name");
                ViewBag.VendorId = new SelectList(vendor, "ID", "Name");
            }
    
            return View();
        }
        [HttpPost]
        public ActionResult  Booking(BookingModel booking, int? productid, int? idvendor)
        {
            if (ModelState.IsValid)
            {
                ...
                model.ShippingStatusId = 20;
                model.PaymentStatusId = 10;
                model.CustomerCurrencyCode = "VNĐ";
                model.CurrencyRate = 1;
                model.CustomerTaxDisplayTypeId = 10;
                model.OrderSubtotalInclTax = 0;
                model.OrderSubtotalExclTax = 0;
                model.OrderSubTotalDiscountInclTax = 0;
                model.OrderSubTotalDiscountExclTax = 0;
                model.OrderShippingInclTax = 0;
                model.OrderShippingExclTax = 0;
                model.PaymentMethodAdditionalFeeInclTax = 0;
                model.PaymentMethodAdditionalFeeExclTax = 0;
                model.OrderTax = 0;
                model.OrderDiscount = 0;
                model.OrderTotal = 0;
                model.RefundedAmount = 0;
                model.RewardPointsWereAdded = false;
                model.CustomerLanguageId = 1;
                model.AffiliateId = 0;
                model.AllowStoringCreditCardNumber = false;
                model.Deleted = false;
                model.CreatedOnUtc = DateTime.Now;
                model.Address = booking.Adress;
                model.VendorsId = booking.VendorId;
                model.Gender = booking.Gender;
                model.ProductId = booking.ProductId;
                model.Phone = booking.Phone;
                model.Name = booking.Name;
                model.Deed = booking.Deed;
                model.Note = booking.Note;
                model.BillingAddressId = 1;
                model.CustomerId = 1;
                _orderService.InsertOrder(model);

                var product = _productService.GetProductById(booking.ProductId);
                var vendor = _vendorService.GetVendorById(booking.VendorId);
                var abc = "";
                if (booking.Deed == 1)
                {
                    abc = "Đăng Ký Lái Thử";
                ...
                if (booking.Gender == false)
                {
                     body = string.Format("<strong>Ông</strong>: {0} - {1} <br /><br />Nhu cầu :{2} <br />Đại lý : {3} <br />Dòng xe: {4}<br />Ghi chú: {5}",
                                            Server.HtmlEncode(booking.Name),
                   Server.HtmlEncode(booking.Phone),
                Server.HtmlEncode(abc),
                   Server.HtmlEncode(vendor.Name), product.Name, booking.Note);
                }
                else
                {
                     body = string.Format("<strong>Bà</strong>: {0} - {1} <br /><br />Nhu cầu :{2} <br />Đại lý : {3} <br />Dòng xe: {4}<br />Ghi chú: {5}",
                                           Server.HtmlEncode(booking.Name),
                  Server.HtmlEncode(booking.Phone),
               Server.HtmlEncode(abc),
                  Server.HtmlEncode(vendor.Name), product.Name,booking.Note);
                ...
                var products1 = _productService.SearchProducts(
                           categoryIds: null,
                           pageSize: int.MaxValue,
                           showHidden: true
                       );
                ...
                var country1 = _countryService.GetCountryById(Convert.ToInt32(234));
                var states1 = _stateProvinceService.GetStateProvincesByCountryId(country1 != null ? country1.Id : 0, _workContext.WorkingLanguage.Id).ToList();
                var vendor1 = _vendorService.GetAllVendors();

                ViewBag.State = new SelectList(states1, "ID", "Name");
                ViewBag.VendorId = new SelectList(vendor1, "ID", "Name");
                ViewBag.ProductId = new SelectList(products1, "ID", "Name");
                TempData["Message"] = "<script>alert('Gửi Thông Tin Đăng Ký Thành Công !');window.location= '/dang-ky-lai-thu'; </script>";

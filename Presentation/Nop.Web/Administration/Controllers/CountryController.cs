...
namespace Nop.Admin.Controllers
{
    public partial class CountryController : BaseAdminController
	{
        ...
        [HttpPost]
        public ActionResult EstimateEditPopup( EstimateModel model)
        {
           
            var sp = _EstimateService.GetEstimateByStateId(model.Id);
            ...
            if (ModelState.IsValid)
            {
                if (sp == null)
                {
                    var spp = new Estimate();
                    spp.LePhiTruocBa = model.LePhiTruocBa;
                    spp.PhiBHVC = model.PhiBHVC;
                    spp.BaoHiemTNDS = model.BaoHiemTNDS;
                    spp.BienSo = model.BienSo;
                    spp.PhiBHVC = model.PhiBHVC;
                    spp.StateId = model.StateId;
                    spp.PhiDuongBo = model.PhiDuongBo;
                    spp.PhiKiemDinh = model.PhiKiemDinh;
                    _EstimateService.InsertEstimate(spp);


                }
                else
                {
                    sp.LePhiTruocBa = model.LePhiTruocBa;
                    sp.PhiBHVC = model.PhiBHVC;
                    sp.BaoHiemTNDS = model.BaoHiemTNDS;
                    sp.BienSo = model.BienSo;
                    sp.PhiBHVC = model.PhiBHVC;
                    sp.StateId = model.StateId;
                    sp.PhiDuongBo = model.PhiDuongBo;
                    sp.PhiKiemDinh = model.PhiKiemDinh;
                    _EstimateService.UpdateEstimate(sp);
                }
